...
export class TreeComponent implements OnInit, ControlValueAccessor, AfterContentInit {
  ...
  constructor(private treeService: TreeService, public Storage: StorageService) {
  }
  ...
  getDataFromServer(id: any, reset?: boolean) {
    ...
    // if (id && id.length > 0) {
    //   let param = { data: this.nodes, ids: id };
    //   this.treeService.getTreeExpand(param).subscribe(result => {
    //     this.nodes = result;
    //     this.excute(reset);
    //     this.loading = false;
    //   });
